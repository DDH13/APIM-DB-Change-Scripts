...
cat <<'EOF' > run-sql-dbscripts.sh
#!/bin/bash
set -e

docker exec -i apim_db_container mysql -u apim_user -papimpass apim_db < ./dbscripts/apimgt/mysql.sql
docker exec -i shared_db_container mysql -u shared_user -psharedpass shared_db < ./dbscripts/mysql.sql
...
chmod +x run-sql-dbscripts.sh
./run-sql-dbscripts.sh
